...
class Event
{
     ...
     *
     */
    public function __construct($setup = null, $checkValidity = true)
    {
        $this->checkValidity = (bool) $checkValidity;
        $this->data = [];

        if (is_array($setup)) {
            if (array_key_exists('id', $setup)) {
                $this->setId($setup['id']);
            }
            if (array_key_exists('name', $setup)) {
                $this->setName($setup['name']);
            }
            if (array_key_exists('retry', $setup)) {
                $this->setRetry($setup['retry']);
            }
            if (array_key_exists('data', $setup)) {
                $this->addData($setup['data']);
            }
        }
    }
